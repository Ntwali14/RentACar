  ...
  <AdminLayout>
    <main class="flex-1 p-8 space-y-6">
      <div class="flex items-center justify-between gap-4">
        <h1 class="text-2xl font-semibold">Customer Disputes</h1>
      </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              v-model="search"
              type="text"
              placeholder="Search by statement, customer, or reservation..."
      ...
      <div class="rounded-md border bg-white overflow-hidden">
        <div v-if="disputes.data.length === 0" class="p-8 text-center text-gray-600">
          No disputes found.
        </div>
        <table v-else class="w-full text-sm">
          <thead class="bg-gray-50 border-b">
            <tr>
              ...
              <th class="px-4 py-3 text-left font-medium text-gray-700">Damage Report</th>
          ...
          <tbody>
            <tr v-for="dispute in disputes.data" :key="dispute.id" class="border-b hover:bg-gray-50">
              <td class="px-4 py-3 font-medium">#{{ dispute.id }}</td>
              <td class="px-4 py-3">{{ dispute.customer?.name }}</td>
              <td class="px-4 py-3">{{ dispute.damageReport?.car?.make }} {{ dispute.damageReport?.car?.model }}</td>
              <td class="px-4 py-3">#{{ dispute.damageReport?.id }}</td>
              <td class="px-4 py-3">
                <span
              ...
              <td class="px-4 py-3 text-xs">{{ new Date(dispute.created_at).toLocaleDateString() }}</td>
              <td class="px-4 py-3">
                <Link :href="route('admin.disputes.show', dispute.id)">
      ...
      <div class="flex items-center justify-between">
        <div class="text-sm text-gray-600">
          Showing {{ disputes.from }} to {{ disputes.to }} of {{ disputes.total }} results
        </div>
        <div class="space-x-2">
          <Link
            v-if="disputes.prev_page_url"
